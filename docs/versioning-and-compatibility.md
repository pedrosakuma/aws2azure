...
| DynamoDB Cosmos stored procedures (`atomicWrite_v2`, frozen `atomicTransactWrite_v2`/`v3`/`v4`, current `atomicTransactWrite_v5`, and `atomicTransactGet_v1`) and their JSON parameter/AST contracts | durable | Stored procedures remain in each container after process replacement. Inventory v4 retains every v2/v3/v4 identity/hash and adds v5 so the durable transaction token window starts at completion. Any body change requires a fresh versioned ID and identity-set version. A create conflict is accepted only after the existing resource's exact id/body is read and verified. | [`DynamoDbPersistedFormatContract`](../src/Aws2Azure.Modules.DynamoDb/Internal/DynamoDbPersistedFormatContract.cs), [`SprocManager`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs) |
