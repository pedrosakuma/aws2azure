...
[`dynamodb-persisted-formats-v3.json`](./compatibility/dynamodb-persisted-formats-v3.json).
Inventory version `3` freezes the current writer versions, every accepted legacy
reader version, the fixture paths, the transaction-idempotency record, and all
current/rollback stored-procedure IDs/body hashes. Inventories v1 and v2 remain
immutable history. A
...
| DynamoDB Cosmos stored procedures (`atomicWrite_v2`, frozen `atomicTransactWrite_v2`/`v3`, current `atomicTransactWrite_v4`, and `atomicTransactGet_v1`) and their JSON parameter/AST contracts | durable | Stored procedures remain in each container after process replacement. Inventory v3 retains every v2/v3 identity/hash and adds v4 for durable transaction idempotency. Any body change requires a fresh versioned ID and identity-set version. A create conflict is accepted only after the existing resource's exact id/body is read and verified. | [`DynamoDbPersistedFormatContract`](../src/Aws2Azure.Modules.DynamoDb/Internal/DynamoDbPersistedFormatContract.cs), [`SprocManager`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs) |
| DynamoDB transaction idempotency records (`_a2a$idempotency$v1$…`, discriminator `transaction-idempotency-v1`) | durable | The record is written atomically with one single-partition transaction, carries a semantic request fingerprint plus server-created/expiry timestamps, and logically expires after ten minutes. Its reserved id cannot be produced by the user-key codec; Query/Scan select only `_a2a = "item"`. Native ttl plus bounded partition-local cleanup prevents continuing growth. | [`TransactWriteItemsHandler`](../src/Aws2Azure.Modules.DynamoDb/Operations/TransactWriteItemsHandler.cs), [`atomicTransactWrite_v4`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.Sources.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs), [`TransactWriteItemsHandlerTests`](../tests/Aws2Azure.UnitTests/DynamoDb/TransactWriteItemsHandlerTests.cs) |
