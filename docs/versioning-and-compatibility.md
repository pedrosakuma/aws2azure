...
[`dynamodb-persisted-formats-v4.json`](./compatibility/dynamodb-persisted-formats-v4.json).
Inventory version `4` freezes the current writer versions, every accepted legacy
reader version, the fixture paths, the transaction-idempotency record, and all
current/rollback stored-procedure IDs/body hashes. Inventories v1 through v3
remain immutable history. A
...
| DynamoDB Cosmos stored procedures (frozen `atomicWrite_v2`, current `atomicWrite_v3`, frozen `atomicTransactWrite_v2`/`v3`/`v4`/`v5`, current `atomicTransactWrite_v6`, and `atomicTransactGet_v1`) and their JSON parameter/AST contracts | durable | Stored procedures remain in each container after process replacement. Inventory v6 retains every frozen identity/hash and adds `atomicWrite_v3`, so a single-write body change still preserves adjacent-runtime compatibility. Any body change requires a fresh versioned ID and identity-set version. A create conflict is accepted only after the existing resource's exact id/body is read and verified. | [`DynamoDbPersistedFormatContract`](../src/Aws2Azure.Modules.DynamoDb/Internal/DynamoDbPersistedFormatContract.cs), [`SprocManager`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs) |
| DynamoDB transaction idempotency records (`_a2a$idempotency$v1$…`, discriminator `transaction-idempotency-v1`) | durable | The record is written atomically with one single-partition transaction, carries a semantic request fingerprint plus server-created/expiry timestamps sampled immediately before token upsert/completion, and logically expires ten minutes after completion. Its reserved id cannot be produced by the user-key codec; Query/Scan select only `_a2a = "item"`. Native ttl plus bounded partition-local cleanup prevents continuing growth. | [`TransactWriteItemsHandler`](../src/Aws2Azure.Modules.DynamoDb/Operations/TransactWriteItemsHandler.cs), [`atomicTransactWrite_v5`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.Sources.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs), [`TransactWriteItemsHandlerTests`](../tests/Aws2Azure.UnitTests/DynamoDb/TransactWriteItemsHandlerTests.cs) |
...
| SQS queue metadata and tags in Service Bus `QueueDescription.UserMetadata` (legacy reader `A2ZSQST1`, current writer `A2ZSQSM2`) | durable | Queue tags plus proxy-owned DelaySeconds / ReceiveMessageWaitTimeSeconds defaults survive process replacement; unknown/foreign metadata must not be overwritten, and the reader preserves compatibility with the legacy tag-only format. | [`SqsQueueTagStore`](../src/Aws2Azure.Modules.Sqs/Operations/SqsQueueTagStore.cs); [`TailHandlersTests`](../tests/Aws2Azure.UnitTests/Sqs/TailHandlersTests.cs#L112-L185), [`QueueMetadataHandlersTests`](../tests/Aws2Azure.UnitTests/Sqs/QueueMetadataHandlersTests.cs) |
