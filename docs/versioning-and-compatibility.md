...
The JSON document selected by `AWS2AZURE_CONFIG_FILE` and its
`AWS2AZURE__...` environment overrides are one public schema. Within a major:
...
|---|---|---|---|
| Binding-centric JSON configuration plus `AWS2AZURE__...` overrides | durable | Operator-owned startup state; the old config must remain usable throughout the supported rollback span. | [`ConfigDocument`](../src/Aws2Azure.Core/Configuration/ConfigDocument.cs#L5-L37), [`Program`](../src/Aws2Azure.Proxy/Program.cs#L89-L97); [`ProxyConfigLoaderTests`](../tests/Aws2Azure.UnitTests/Configuration/ProxyConfigLoaderTests.cs#L22-L177) |
...
| SNS subscription JSON in Service Bus `SubscriptionDescription.UserMetadata` | durable | Protocol, endpoint, filter policy/scope, and raw-delivery state survive process replacement. | [`SnsSubscriptionSupport`](../src/Aws2Azure.Modules.Sns/Operations/SnsSubscriptionSupport.cs#L68-L89), [`serialization`](../src/Aws2Azure.Modules.Sns/Operations/SnsSubscriptionSupport.cs#L203-L233); [`SubscribeHandlerTests`](../tests/Aws2Azure.UnitTests/Sns/SubscribeHandlerTests.cs#L157-L188) |
...
| Metadata/token caches, AMQP pools, circuit-breaker state, and fallback process signing keys | ephemeral | Rebuilt after restart; no compatibility or restore guarantee. Authoritative state remains in configuration, clients, or Azure. | [`TableMetadataCache`](../src/Aws2Azure.Modules.DynamoDb/Internal/TableMetadataCache.cs#L7-L95), [`EventHubMetadataCache`](../src/Aws2Azure.Modules.Kinesis/EventHubsRest/EventHubMetadataCache.cs#L16-L108), [`ShardIteratorTokenCodecFactory`](../src/Aws2Azure.Modules.Kinesis/ShardIterators/ShardIteratorTokenCodecFactory.cs#L7-L43); [`EventHubMetadataCacheTests`](../tests/Aws2Azure.UnitTests/Kinesis/EventHubMetadataCacheTests.cs#L10-L83), [`ShardIteratorTokenCodecTests`](../tests/Aws2Azure.UnitTests/Kinesis/ShardIteratorTokenCodecTests.cs#L161-L178) |
