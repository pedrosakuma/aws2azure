...
[`dynamodb-persisted-formats-v2.json`](./compatibility/dynamodb-persisted-formats-v2.json).
Inventory version `2` freezes the current writer versions, every accepted legacy
reader version, the fixture paths, and both current and rollback
stored-procedure IDs/body hashes. Inventory v1 remains immutable history. A
...
| DynamoDB Cosmos stored procedures (`atomicWrite_v2`, rollback `atomicTransactWrite_v2`, current `atomicTransactWrite_v3`, and `atomicTransactGet_v1`) and their JSON parameter/AST contracts | durable | Stored procedures remain in each container after process replacement. Inventory v2 retains the v2 transaction identity/hash for rollback and pins every current body. Any body change requires a fresh versioned ID and identity-set version. A create conflict is accepted only after the existing resource's exact id/body is read and verified. | [`DynamoDbPersistedFormatContract`](../src/Aws2Azure.Modules.DynamoDb/Internal/DynamoDbPersistedFormatContract.cs), [`SprocManager`](../src/Aws2Azure.Modules.DynamoDb/Internal/SprocManager.cs); [`PersistedFormatCompatibilityTests`](../tests/Aws2Azure.UnitTests/DynamoDb/Persistence/PersistedFormatCompatibilityTests.cs) |
