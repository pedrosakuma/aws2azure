...
REST and AMQP transports differ beyond FIFO: receipt-handle formats, the mechanism used for VisibilityTimeout=0 immediate release (REST via Unlock Message, AMQP via Abandon), per-entry partial-failure granularity on batch sends (real on AMQP, coarser on REST), and dead-letter attribution (AMQP only).
