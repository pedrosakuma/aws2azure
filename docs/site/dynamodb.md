...
- Known limitation: a user attribute literally named `ttl` collides with Cosmos' reserved `ttl` document field (a pre-existing latent issue for any reserved system field). Avoid naming the TTL attribute `ttl`.
