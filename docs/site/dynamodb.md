...
- GSI Query (opt-in `DynamoDb.EnableGlobalSecondaryIndexQueries=true`) is a cross-partition Cosmos query, so unlike a base-table or LSI Query it fans out across partitions rather than honoring DynamoDB's single-partition guarantee; this is inherent to GSIs (which have their own partition key) and trades RU/latency for the index lookup. GSIs are eventually consistent in DynamoDB, so ConsistentRead=true is rejected (ValidationException). GSI ordering follows the same Cosmos-collation caveat as LSI (raw-attribute ORDER BY: S code-point collation vs DDB UTF-8 byte order; `_a2a:N` envelope values not ordered numerically); a hash-only GSI returns rows unordered (matching DynamoDB, which only orders within a GSI partition when the index is composite). GSI Query ScannedCount reflects rows after the Cosmos prefilter of the key condition (and the IS_DEFINED membership guard), not DynamoDB's pre-filter evaluated-items count; the base-table aggregate recovery is skipped for GSI queries (the count scope is cross-partition on the index key attribute, not `c.id`). Correct ordering via synthetic order-preserving encoded index fields ("Option B") is deferred to a later PR.
- Smoke-verified against the Cosmos DB Linux emulator (vNext preview) via Testcontainers; not yet exercised against real Azure Cosmos DB. LSI Query is emulator-verified only; not yet exercised against real Azure Cosmos DB. GSI Query (behind the opt-in flag) is emulator/unit-verified only and MUST be validated against real Azure Cosmos DB before it can be promoted past `partial` — cross-partition ORDER BY, pagination/continuation across partitions, and eventual-consistency behavior are exactly the areas where the emulator diverges from real Azure, which is why the capability ships default-off.
