...
- The first complete compatible protected-main runtime is recorded as a rollback-only bootstrap. The known v2 runtime remains ineligible because it uses independent reads and lacks the durable ClientRequestToken contract. A later distinct runtime must pass exact candidate-to-bootstrap rollback correctness, three production-shaped load runs, and final review before any promotion, candidate, approved, or GA profile claim.
