...
- Transaction execution POSTs opt out of proxy-level automatic retries because a lost response is ambiguous without durable ClientRequestToken deduplication. Stored-procedure availability is cached per Cosmos account/database/container, invalidated on table lifecycle, and evicted/reprovisioned once after an execution 404.
- `atomicWrite_v2` remains byte-identical. `atomicTransactWrite_v2` remains in the additive persisted-format identity set for future adjacent-runtime format checks, but it is not a workload-profile rollback target. The current condition/cancellation contract uses `atomicTransactWrite_v3`; TransactGet uses `atomicTransactGet_v1`.
- The Cosmos Linux emulator cannot execute these scripts. V3 write semantics, v1 snapshots, contention, restart, and exact-body conflict handling require fresh real-Azure runs before new seals are committed.
- No trusted prior release implements the complete profile: the known v2 runtime uses independent reads and does not enforce ClientRequestToken rejection. It is not an approved bootstrap/rollback target; qualification is rollout-only and records the adjacent-runtime row as skipped/inconclusive until a compatible prior release exists.
