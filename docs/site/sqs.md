...
| Attribute.RedrivePolicy | 🟡 partial | — | SQS RedrivePolicy JSON ({deadLetterTargetArn,maxReceiveCount}) is parsed and mapped to SB ForwardDeadLetteredMessagesTo (queue-name segment of the ARN) + MaxDeliveryCount. maxReceiveCount is bounded to 1..1000 per SQS. Service Bus exposes the delivery whose count first exceeds MaxDeliveryCount and evaluates dead-letter forwarding on the next source receive after that delivery is released or abandoned, so native redrive permits one more source delivery than SQS. The target DLQ must already exist (auto-provisioning is intentionally not implemented; client owns DLQ lifecycle). |  |  |
...
- Service Bus MaxDeliveryCount is not an exact SQS maxReceiveCount boundary: a message configured with maxReceiveCount=1 is delivered with counts 1 and 2, then dead-lettered when the source is polled after the second delivery is released. Exact enforcement would require proxy-owned durable queue metadata and receive-time dead-lettering; the current direct broker mapping intentionally documents the native one-delivery offset.
...
| MaxResults / NextToken pagination | ✅ implemented | — | MaxResults defaults to 1000 (SQS hard cap); NextToken is a stateless integer cursor into the SB queue listing. The proxy emits it only when another matching source exists, and the cursor survives proxy restart. |  |  |
...
| 60s cool-down (PurgeQueueInProgress) | 🟡 partial | — | Enforced by a bounded in-process tracker keyed by namespace+queue. Expired and failed attempts are removed opportunistically; cross-replica coordination remains unsupported. |  |  |
...
| FIFO MessageGroupId session receive | 🟡 partial | — | The AMQP attach now matches the official Azure .NET/Java session receiver shape: a bare string/null session-filter value, receiver settle mode second, a local target terminus, and a uint com.microsoft:timeout property for broker-assigned sessions. Positive real-Azure evidence remains required in the separate sqs-fifo-amqp profile; standard messaging is unaffected. |  |  |
...
- Prior real-Azure FIFO attempts exposed incomplete session attach parity. Source now emits the bare filter shape used by official Azure .NET/Java clients and includes their receiver settlement, target terminus, and AcceptNextSession timeout fields; described and bare filter values remain accepted on decode. The sqs-fifo-amqp real-Azure scenario must produce positive evidence before this sub-feature is promoted; those optional results do not affect sqs-standard-messaging.
...
- Native redrive boundary: Service Bus exposes one additional source delivery compared with SQS maxReceiveCount. With MaxDeliveryCount=1, delivery counts 1 and 2 are observable; after the second delivery is released, the next source receive evaluates the dead-letter transition and forwarding. Exact SQS enforcement requires durable proxy-owned redrive metadata and receive-time dead-lettering rather than direct broker mapping.
...
| RedrivePolicy → ForwardDeadLetteredMessagesTo | 🟡 partial | — | JSON is parsed and mapped to ForwardDeadLetteredMessagesTo + MaxDeliveryCount. SB read-merge-write replaces the whole queue entity so the patch is preserved across subsequent SetQueueAttributes calls. Service Bus dead-letters after one additional exposed delivery compared with the SQS maxReceiveCount boundary. |  |  |
...
- Service Bus MaxDeliveryCount is not an exact SQS maxReceiveCount boundary: the delivery whose count first exceeds MaxDeliveryCount remains receivable and is dead-lettered after it is released or abandoned. Exact enforcement requires proxy-owned durable queue metadata plus receive-time dead-lettering.
