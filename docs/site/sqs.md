...
| FIFO MessageGroupId session receive | 🟡 partial | — | Real Azure AcceptNextSession currently waits until timeout even when one FIFO group has just been sent successfully; earlier concurrent two-group runs failed on the same acquisition path. The conformance matrix retains this as explicit optional coverage outside the standard messaging profile. |  |  |
