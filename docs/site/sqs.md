...
- RedrivePolicy maps to Service Bus MaxDeliveryCount and ForwardDeadLetteredMessagesTo. Because Service Bus may transfer the first over-limit delivery, the AMQP receive path checks persisted queue metadata on redelivery and explicitly dead-letters that delivery before exposing it to the AWS client. The legacy REST receive transport retains the native additional delivery.
...
- AMQP redrive boundary: Service Bus may transfer a message whose delivery count has already exceeded MaxDeliveryCount. Before returning an AMQP delivery to the AWS client, the proxy reads the queue's management metadata on redelivery; when the current receive count exceeds RedrivePolicy.maxReceiveCount, it explicitly dead-letters the delivery so Service Bus forwarding occurs without exposing the extra source receive. First deliveries and queues without redelivery do not incur this management lookup. The legacy REST receive transport cannot issue an explicit dead-letter settlement and retains the native Service Bus boundary.
...
- Service Bus may transfer the delivery whose count first exceeds MaxDeliveryCount. The AMQP transport closes that native boundary difference by reading queue metadata only on redelivery and explicitly dead-lettering an over-limit message before it is returned by ReceiveMessage. The legacy REST receive transport cannot express that settlement and retains the additional delivery.
