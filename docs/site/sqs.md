...
| FIFO MessageGroupId session receive | 🟡 partial | — | The AMQP source filter now uses Service Bus's required described session-filter value (descriptor 0x000001370000000C), fixing the code-level AcceptNextSession filter mismatch. Positive real-Azure evidence remains required in the separate sqs-fifo-amqp profile; standard messaging is unaffected. |  |  |
...
- The prior real-Azure FIFO failures (runs 29472731381, 29471929057, and 29471188679) used a bare session-filter value. Source now emits the Service Bus described filter shape used by the Azure SDK. The new sqs-fifo-amqp real-Azure scenario must produce positive evidence before this sub-feature is promoted; those optional results do not affect sqs-standard-messaging.
