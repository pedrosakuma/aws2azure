...
| Concurrent FIFO MessageGroupId consumers | 🟡 partial | — | A single real-Azure session preserves group order and deduplication. Concurrent AcceptNextSession calls for two ready groups currently leave the second call waiting until timeout; the conformance matrix retains this as an explicit skipped scenario so ReceiveMessage is not eligible for a real-Azure verification seal. |  |  |
...
- Single-group FIFO order and deduplication are exercised against real Azure. Concurrent acquisition of two ready MessageGroupId sessions remains an observed production-Azure gap: runs 29471929057 and 29471188679 timed out the second AcceptNextSession call, so the conformance scenario remains explicitly skipped and blocks seal eligibility.
...
- Emulator divergence (Phase 2.7 Slice 6): the Azure Service Bus emulator does NOT echo the broker-assigned session-id back in the receiver attach response's `com.microsoft:session-filter` when the client requests `sessionId=null` (broker-assigned). The proxy's FIFO receive path correctly fails fast in that case with `InvalidOperationException`, so FIFO end-to-end coverage against the emulator is impossible. Real Azure Service Bus echoes the assigned id for a single acquired session; concurrent acquisition of a second ready group remains the separate gap documented above.
