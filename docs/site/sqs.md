...
- Service Bus MaxDeliveryCount is not an exact SQS maxReceiveCount boundary: a message configured with maxReceiveCount=1 is delivered with counts 1 and 2, then dead-lettered after the second delivery is released. Exact enforcement would require proxy-owned durable queue metadata and receive-time dead-lettering; the current direct broker mapping intentionally documents the native one-delivery offset.
