...
| FIFO MessageGroupId session receive | 🟡 partial | — | The AMQP source filter uses the service-compatible legacy described session-filter value (descriptor 0x000000137000000C) emitted by the official Azure Go SDK. Positive real-Azure evidence remains required in the separate sqs-fifo-amqp profile; standard messaging is unaffected. |  |  |
...
- Prior real-Azure FIFO attempts exposed session-filter wire incompatibilities. Source now emits descriptor 0x000000137000000C, the legacy numeric shape used by the official Azure Go SDK, while accepting described and bare values on decode. The sqs-fifo-amqp real-Azure scenario must produce positive evidence before this sub-feature is promoted; those optional results do not affect sqs-standard-messaging.
