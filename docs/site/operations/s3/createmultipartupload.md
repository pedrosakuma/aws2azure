...
- Applying x-amz-tagging (object-tagging sub-feature) is a second, non-atomic Azure call (Set Blob Tags) issued after CompleteMultipartUpload's block-list commit succeeds, unlike Content-Type/x-amz-meta-*, which ride along on the commit request itself. If Set Blob Tags fails, CompleteMultipartUpload returns an error even though the blob is already committed with its content and standard headers; the durable state record is left leased (not retired) rather than deleted, so the lease simply expires and a client retry of CompleteMultipartUpload can re-attempt tag application.
