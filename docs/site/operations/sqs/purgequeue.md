...
Cross-replica coordination is now implemented: the cool-down deadline is persisted in Service Bus QueueDescription.UserMetadata (the same compact envelope used for queue tags/defaults) and started with a GET + PUT If-Match compare-and-swap — a losing replica's PUT gets 412 PreconditionFailed and re-reads, at which point it observes either the winner's already-active deadline (PurgeQueueInProgress) or an expired one it can now claim. A bounded in-process tracker layers a same-replica fast reject in front of this (no network round trip for an immediate repeat). If the drain that follows a successful claim then fails (queue deleted mid-purge, a peek-lock/delete error, or an unexpected exception), the failed attempt rolls back both the local reservation and the just-written distributed marker via a best-effort GET + PUT-with-ETag clear, so a client that received an error (no purge actually started) is not incorrectly blocked from retrying for up to 60 seconds; the rollback only clears the marker if it still matches what this call wrote, so it can never clobber a legitimate cool-down a concurrent replica started afterwards. The remaining gap is a narrow, explicit fallback to single-replica, in-process-only coordination when the queue's UserMetadata is already owned by non-aws2azure content or too full (near the ~1024 character Service Bus UserMetadata limit) to fit the cool-down marker alongside existing tags; that fallback path is still tracked pending a genuinely dependency-free way to signal cooldown when UserMetadata is unavailable for aws2azure's use.
...
- Cooldown state is hard-capped and failed/nonexistent-queue attempts release their local reservation. When the local cap is occupied by active purges, new queue keys receive a retryable ServiceUnavailable response rather than growing process state; this local cap is independent of, and layered in front of, the distributed Service-Bus-persisted deadline. A drain failure that occurs *after* the distributed marker was already committed also triggers a best-effort rollback clearing that marker (matched against the exact deadline this call wrote, so it never clobbers a legitimate cool-down claimed by a concurrent replica in the meantime), keeping the two reservations symmetric — a failed purge attempt never leaves a live cross-replica cool-down behind.
