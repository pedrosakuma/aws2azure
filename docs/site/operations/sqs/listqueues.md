...
- Real AWS SQS's Query (legacy XML) protocol front door emits an empty <Detail/> element in every <ErrorResponse> body (e.g. auth-stage InvalidClientTokenId / SignatureDoesNotMatch and dispatch-stage InvalidAction). The proxy's shared Query error envelope omits the element because SDK deserializers do not consume it — its presence in real AWS is a legacy Query-protocol convention with no functional purpose. [conformance:query-invalid-action::missing-field:Detail] [conformance:query-invalid-signature::missing-field:Detail] [conformance:query-unrecognized-client::missing-field:Detail]
