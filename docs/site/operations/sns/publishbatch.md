...
- On the Event Grid backend, PublishBatch targets classic Event Grid custom topics. Regional recovery for those topics is metadata-only and Microsoft-managed on a best-effort timeline, so the proxy cannot provide rapid, customer-controlled regional failover by itself.
