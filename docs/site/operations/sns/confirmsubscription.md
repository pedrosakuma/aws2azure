...
- Service Bus Geo-DR can replay this compatibility metadata only when the namespace/endpoints use the Geo-DR alias and the pairing has replicated the latest entity changes; it still does not preserve queued messages already held in the backing Service Bus subscription.
