...
- Supported protocols are validated only by protocol name plus a non-empty Endpoint value. aws2azure does not verify SQS ARN shape, HTTP/HTTPS URL syntax, or endpoint reachability during Subscribe.
- When credentials.serviceBusTopics.topics[*].serviceBusTopicName is configured for an exact SNS topic name, Subscribe stores the Azure subscription under that mapped Service Bus topic path while keeping the SNS-facing SubscriptionArn on the original topic name.
- RawMessageDelivery is metadata-only compatibility state. Subscribe stores the flag for later Get/SetSubscriptionAttributes round-trips, but publish-time delivery shape does not change: Service Bus publishes remain raw message bytes and Event Grid publishes keep their Event Grid envelope.
...
- Service Bus Geo-DR only preserves this profile when the namespace/endpoints are configured through the Geo-DR alias. The subscription entity and its UserMetadata replicate as metadata, but queued messages already retained in the subscription do not; workloads needing preserved backlog across regions must use Geo-Replication instead.
