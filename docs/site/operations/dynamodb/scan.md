# dynamodb / Scan {#operation-dynamodb-scan}

[← dynamodb operation index](../../dynamodb.md) · [Coverage matrix](../../coverage.md)

- **Capability ID:** `operation:dynamodb:scan`
- **Status:** 🟡 partial
...
- **Capability ID:** `sub-feature:dynamodb:scan:expressionattributenames---expressionattributevalues`
- **Status:** ✅ implemented

### Limit {#sub-feature-limit}

- **Capability ID:** `sub-feature:dynamodb:scan:limit`
- **Status:** ✅ implemented

Caps the *scanned* (pre-filter) row count when the filter is residual-only; pageSize is sized to the remaining evaluation budget so the per-page continuation never skips rows. When a FilterExpression is pushed (fully or partially) into the Cosmos SQL, Cosmos pre-filters at the storage layer; an unbounded scan recovers the faithful ScannedCount via a server-side count, but a Limit cannot be reconciled with server-side pre-filtering — see behavior_differences for the page-boundary trade-off.

### ExclusiveStartKey / LastEvaluatedKey {#sub-feature-exclusivestartkey---lastevaluatedkey}

- **Capability ID:** `sub-feature:dynamodb:scan:exclusivestartkey---lastevaluatedkey`
- **Status:** ✅ implemented

Pagination round-trips the Cosmos `x-ms-continuation` token inside a sentinel attribute `__a2a_continuation` (typed-string `S`). Most AWS SDKs treat LastEvaluatedKey as opaque and pass it back verbatim.

### ConsistentRead {#sub-feature-consistentread}

- **Capability ID:** `sub-feature:dynamodb:scan:consistentread`
- **Status:** ✅ implemented

Forwards `x-ms-consistency-level: Strong` for the Cosmos query when true; account-level consistency cap still applies. Opt-in startup probe (`DynamoDb.ConsistencyCheck` = Warn/Required, #204) flags accounts that cannot honor Strong at boot.

### Select {#sub-feature-select}

- **Capability ID:** `sub-feature:dynamodb:scan:select`
- **Status:** 🟡 partial
- **Disposition:** 🔵 by design
...
### IndexName (GSI / LSI) {#sub-feature-indexname--gsi---lsi}

- **Capability ID:** `sub-feature:dynamodb:scan:indexname--gsi---lsi`
- **Status:** 🟡 partial
- **Disposition:** 🔵 by design
- **Real-Azure verified:** ✅ 2026-07-24 · [evidence](https://github.com/pedrosakuma/aws2azure/actions/runs/30059183242) · [workflow run](https://github.com/pedrosakuma/aws2azure/actions/runs/30059183242)
...
### Parallel scan (Segment / TotalSegments) {#sub-feature-parallel-scan--segment---totalsegments}

- **Capability ID:** `sub-feature:dynamodb:scan:parallel-scan--segment---totalsegments`
- **Status:** ⛔ unsupported
- **Disposition:** ⚫ non-goal

Rejected with ValidationException. Cosmos cross-partition queries fan out internally; explicit per-segment parallelism is deferred to a later slice.

### Legacy ScanFilter / ConditionalOperator / AttributesToGet {#sub-feature-legacy-scanfilter---conditionaloperator---attributestoget}

- **Capability ID:** `sub-feature:dynamodb:scan:legacy-scanfilter---conditionaloperator---attributestoget`
- **Status:** ⛔ unsupported
- **Disposition:** ⚫ non-goal

Legacy v1 parameters are rejected loudly with ValidationException — use FilterExpression / ProjectionExpression.

### ReturnConsumedCapacity {#sub-feature-returnconsumedcapacity}

- **Capability ID:** `sub-feature:dynamodb:scan:returnconsumedcapacity`
- **Status:** ⛔ unsupported
- **Disposition:** 🔵 by design
