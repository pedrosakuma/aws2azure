...
- Conditional / ReturnValues=NONE updates use the single-item `atomicWrite_v3` Cosmos stored procedure when stored procedures are enabled AND the expression is within the sproc's supported subset: the condition is evaluated and the UpdateExpression applied server-side in one atomic round-trip. **Validated against real Azure Cosmos DB** (Strong consistency). The v3 body fixes the emulator-missed read-link defect (`SELECT * FROM c WHERE c.id = @id` instead of an invalid `getSelfLink() + 'docs/' + id` mixed link), serialises SET-value operands as `$k`-tagged envelopes (`lit`/`path`/`op`/`ifne`/`lap`) so the sproc resolves arithmetic (`a = a + :i`), attribute copies, `if_not_exists` and `list_append` server-side instead of storing the unresolved AST, and rejects oversized (>400 KiB) base items before commit.
...
- Even when the request itself is otherwise sproc-eligible, tables with DynamoDB TTL enabled or with any N-typed secondary-index sort key silently bypass `atomicWrite_v3` and fall back to the non-atomic GET → modify → PUT loop (including when stored-procedure mode is `Required`). The current sproc cannot recompute Cosmos' native per-item `ttl` or the synthetic `_a2a$ord$<attr>` fields server-side after the update, so this is an accepted correctness gap until the server-side write contract is redesigned.
- On tables with one or more local secondary indexes, the `atomicWrite_v3` path validates only the merged base item's size. It does **not** enforce DynamoDB's stricter "base item + every LSI entry" combined 400 KiB cap because the sproc executes without table/index projection metadata; the non-sproc GET → modify → PUT path still enforces the combined limit in C#. This is an accepted gap until the single-write sproc contract can accept enough metadata to reproduce `DynamoDbItemSize.TryCalculateWithLocalSecondaryIndexes(...)` server-side.
- Smoke-verified against the Cosmos DB Linux emulator (vNext preview) via Testcontainers; the GET → modify → PUT fallback path is emulator-covered, the `atomicWrite_v3` sproc path is validated against real Azure Cosmos DB.
