...
- **Status:** ✅ implemented
- **Real-Azure verified:** ✅ 2026-08-27 · [evidence](https://github.com/pedrosakuma/aws2azure/actions/runs/33048633648) · [workflow run](https://github.com/pedrosakuma/aws2azure/actions/runs/33048633648)

`atomicTransactWrite_v6` adds an Update action alongside Put/Delete/ConditionCheck, executed atomically in the same single-partition Cosmos transaction, reusing `atomicWrite_v3`'s server-side UpdateExpression executor. Supported subset mirrors non-transactional UpdateItem's stored-procedure-eligible slice: SET (including arithmetic and `if_not_exists`/`list_append`) and REMOVE on one non-reserved top-level attribute per clause, with S/BOOL/NULL/native-number/map/list literals only. ADD, DELETE, nested/list-index paths, dotted names, and Cosmos system field names (e.g. `_etag`) are rejected with ValidationException before stored-procedure I/O, matching UpdateItem's own ineligibility rules. Requests are rejected outright (no C# fallback) when the table has TTL enabled or a numeric GSI sort key, since Update has no non-sproc code path in this operation. Key attributes present on the item are silently re-stamped onto the updated document, mirroring UpdateItem's `ReinforceKeyAttributes` behavior. Verified against real Azure Cosmos DB by the transaction-specific real-Azure conformance scenario added for issue #798.
...
- The Cosmos Linux emulator cannot execute these scripts, so transaction stored-procedure behavior is verified by dedicated real-Azure scenarios rather than emulator coverage.
