...
`atomicTransactWrite_v6` adds an Update action alongside Put/Delete/ConditionCheck, executed atomically in the same single-partition Cosmos transaction, reusing `atomicWrite_v3`'s server-side UpdateExpression executor. Supported subset mirrors non-transactional UpdateItem's stored-procedure-eligible slice: SET (including arithmetic and `if_not_exists`/`list_append`) and REMOVE on one non-reserved top-level attribute per clause, with S/BOOL/NULL/native-number/map/list literals only. ADD, DELETE, nested/list-index paths, dotted names, and Cosmos system field names (e.g. `_etag`) are rejected with ValidationException before stored-procedure I/O, matching UpdateItem's own ineligibility rules. Requests are rejected outright (no C# fallback) when the table has TTL enabled or a numeric GSI sort key, since Update has no non-sproc code path in this operation. Key attributes present on the item are silently re-stamped onto the updated document, mirroring UpdateItem's `ReinforceKeyAttributes` behavior. Unit-tested (parsing, eligibility gating, idempotency fingerprint, request-shape assembly); the new stored procedure body itself has *not* been exercised against a live Cosmos DB (emulator or real Azure) in this change — see the emulator caveat below.
...
NONE and ALL_OLD are accepted (per-operation, on Put/Delete/ConditionCheck/Update); any other value is rejected with ValidationException. `atomicTransactWrite_v6` captures each item's pre-write Cosmos snapshot atomically with the cancellation outcome inside the same script invocation — the proxy still never races a post-failure GET. On a cancelled transaction, the CancellationReasons entry for any operation that requested ALL_OLD carries an `Item` populated from that atomic snapshot, converted through the same envelope logic GetItem uses so the two never diverge; operations that did not request ALL_OLD never surface an item, even if the script captured one. Unit-tested for request-shape assembly and cancellation-reason translation; the stored procedure's actual snapshot-then-cancel behavior has *not* been exercised against a live Cosmos DB (emulator or real Azure) in this change — see the emulator caveat below.
...
- `atomicWrite_v2`, `atomicTransactWrite_v2`, `atomicTransactWrite_v3`, `atomicTransactWrite_v4`, and `atomicTransactWrite_v5` remain frozen in the additive persisted-format identity set. The current single-write identity is `atomicWrite_v3`; the current durable idempotency contract uses `atomicTransactWrite_v6`, which adds the Update action and ReturnValuesOnConditionCheckFailure=ALL_OLD snapshot capture on top of v5's condition/idempotency semantics; TransactGet uses `atomicTransactGet_v1`.
- The Cosmos Linux emulator cannot execute these scripts, and no such emulator (nor real Azure) was reachable in the sandbox that authored `atomicTransactWrite_v6`. V5/v6 write/idempotency semantics, the new Update action, ALL_OLD snapshot capture, v1 snapshots, contention, restart, and exact-body conflict handling remain real-Azure-only (or at minimum emulator-only) test surfaces pending verification; this change is validated by unit tests and code review only.
