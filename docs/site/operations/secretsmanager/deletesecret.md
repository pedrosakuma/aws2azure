# secretsmanager / DeleteSecret {#operation-secretsmanager-deletesecret}

[← secretsmanager operation index](../../secretsmanager.md) · [Coverage matrix](../../coverage.md)

- **Capability ID:** `operation:secretsmanager:deletesecret`
- **Status:** ✅ implemented
- **Azure equivalent:** `DELETE https://{vault}.vault.azure.net/secrets/{name}`
- **Real-Azure verified:** ✅ 2026-07-16 · [evidence](https://github.com/pedrosakuma/aws2azure/actions/runs/29473539261) · [workflow run](https://github.com/pedrosakuma/aws2azure/actions/runs/29473539261)

## Behaviour differences

- Initial MVP uses Key Vault AAD auth and translates the core secret CRUD/read paths to AWS Secrets Manager JSON responses.
- Advanced rotation, restore, and policy semantics are not yet modeled; the proxy uses Key Vault secret versions as the AWS version surface.
- Responses use the AWS JSON 1.1 wire shape (Unix-epoch numeric timestamps, Content-Type application/x-amz-json-1.1); validated end-to-end against a real Azure Key Vault through the proxy with the AWS SDK.
- Plain DeleteSecret now reports Key Vault's actual deleted/scheduled-purge timestamps instead of fabricating response dates locally. Real-AWS and real-Azure Tier-3 captures therefore cannot compare exact `DeletionDate` bytes meaningfully: AWS and Key Vault schedule deletion independently and at different capture times, so the offline diff allow-lists only the field value while still asserting the field is present and numeric. [conformance:create-get-update-delete-secret-roundtrip::field-value:DeletionDate] [conformance:describe-secret-roundtrip::field-value:DeletionDate] [conformance:list-secrets-pagination::field-value:DeletionDate]
- ForceDeleteWithoutRecovery now maps to Key Vault delete followed by purge when the target vault permits purge. If the vault still enforces retention (for example, purge protection is enabled) or the configured identity lacks purge permission, aws2azure returns an explicit error instead of silently soft-deleting.
- ForceDeleteWithoutRecovery's purge convergence is asynchronous, matching AWS's own documented DeleteSecret contract: the secret is made inaccessible and one purge attempt runs before the response is written, but if Key Vault's soft-delete state has not yet converged to purgeable, the remaining bounded-retry attempts continue on a background continuation after the 200 response is sent, instead of blocking the caller on Key Vault's backend purge latency. Deterministic failures detected on that first attempt (missing purge permission, a non-purgeable/purge-protected vault) still block the response with an AWS error, unchanged from before; failures that only manifest on a later background attempt are logged, not surfaced to the caller, matching AWS's documented behavior of not reporting asynchronous purge outcomes. [conformance:HandleAsync_DeleteSecret_force_delete_defers_purge_convergence_to_background]
- RecoveryWindowInDays is accepted so common AWS cleanup flows keep working, but the requested window is not enforced. Key Vault uses the vault's fixed soft-delete retention policy and DeleteSecret returns the actual Key Vault scheduled-purge timestamp rather than the caller-requested window.
- Certificate-backed Key Vault secrets (`managed: true`) cannot be deleted through the secrets API. When Key Vault rejects DELETE with HTTP 405, aws2azure returns InvalidRequestException with a certificate-specific message instead of a generic parameter error.

## References

- <https://docs.aws.amazon.com/secretsmanager/latest/apireference/API_DeleteSecret.html>
- <https://learn.microsoft.com/rest/api/keyvault/secrets/delete-secret>
- <https://learn.microsoft.com/rest/api/keyvault/secrets/purge-deleted-secret/purge-deleted-secret>
- <https://github.com/pedrosakuma/aws2azure/issues/991>
- <https://github.com/pedrosakuma/aws2azure/issues/993>

