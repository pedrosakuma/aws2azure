...
- Exact `CreatedDate` / `LastChangedDate` values from Tier-3 real-diff runs are intentionally non-comparable across real AWS and real Azure captures. The two captures happen at different wall-clock times, and Key Vault surfaces second-granularity timestamps, so the conformance gate keeps asserting field presence/shape while allow-listing only the exact value bytes for this case. [conformance:describe-secret-roundtrip::field-value:CreatedDate] [conformance:describe-secret-roundtrip::field-value:LastChangedDate]
