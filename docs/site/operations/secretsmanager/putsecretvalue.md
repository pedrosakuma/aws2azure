...
- New versions inherit the current secret's caller-visible tags before aws2azure's version-metadata tags are overlaid, so TagResource/UntagResource changes survive later PutSecretValue writes.
- Exactly one of SecretString or SecretBinary must be supplied; mixed or missing value fields are rejected with InvalidParameterException instead of silently preferring binary.
