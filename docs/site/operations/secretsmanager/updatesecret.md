...
- New versions inherit the current secret's caller-visible tags before aws2azure's version metadata is overlaid, so TagResource/UntagResource changes survive later UpdateSecret writes.
- When UpdateSecret includes a new secret value, exactly one of SecretString or SecretBinary must be supplied; mixed value fields are rejected with InvalidParameterException instead of silently preferring binary.
- Metadata-only UpdateSecret requests (for example, Description without SecretString/SecretBinary) are rejected with NotImplementedException instead of being silently approximated. Azure Key Vault's version update contract does not expose the AWS description-only metadata path on its own.
