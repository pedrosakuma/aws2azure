# secretsmanager / GetSecretValue {#operation-secretsmanager-getsecretvalue}

[← secretsmanager operation index](../../secretsmanager.md) · [Coverage matrix](../../coverage.md)

- **Capability ID:** `operation:secretsmanager:getsecretvalue`
- **Status:** ✅ implemented
...
- Exact `CreatedDate` values from credential-free Tier-3 real-diff runs are intentionally non-comparable even after the wire shape matches AWS: the AWS golden and the Azure-backed evidence are captured at different real times, and Key Vault exposes created timestamps only at one-second granularity. Presence and numeric epoch encoding still matter, but the exact wall-clock value is documented backend drift rather than a proxy bug. [conformance:create-get-update-delete-secret-roundtrip::field-value:CreatedDate]
