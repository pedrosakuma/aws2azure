...
| per-part-etag-validation | ⛔ unsupported | — | The proxy still cannot validate client-supplied per-part ETags against Azure's staged blocks because Azure exposes no equivalent per-block MD5 lookup. This remains an explicit permanent gap. |  |  |
| minimum-part-size | ⛔ unsupported | — | S3's 5 MiB minimum for non-final parts is not enforced. |  |  |
...
| server-side-encryption | ⛔ unsupported | — | x-amz-server-side-encryption* request headers are ignored. Encryption remains governed by the destination storage account. |  |  |
| object-tagging | ⛔ unsupported | — | x-amz-tagging is ignored on initiate. |  |  |
| object-lock | ⛔ unsupported | — |  |  |  |
| storage-class | ⛔ unsupported | — | x-amz-storage-class is ignored; Azure uses the account/container defaults. |  |  |
...
| source-conditional-headers | 🟡 partial | — | Date-based x-amz-copy-source-if-* headers forward to Azure. Concrete-ETag x-amz-copy-source-if-match / x-amz-copy-source-if-none-match remain unsupported because proxy-translated S3 ETags do not round-trip into Azure's raw ETag space; '*' is still honoured. |  |  |
