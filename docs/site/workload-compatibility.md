...
|---|---|---:|---:|---:|---:|---:|
| [dynamodb](dynamodb.md) | Available | 7 | 12 | 0 | 0 | 14/19 |
| [kinesis](kinesis.md) | Available | 0 | 7 | 0 | 0 | 7/7 |
| [s3](s3.md) | Available | 27 | 23 | 7 | 17 | 30/74 |
...
| Single-partition transactions | 🟡 conditional | 2 partial | 2/2 | TransactGetItems and TransactWriteItems can be atomic only when every item belongs to one table and one logical partition.<br>Co-locate all transaction items under one partition key, test the supported condition-expression subset, and use a single-write account or configure preferredRegions[0] identically across replicas with one authoritative writable region.<br>[TransactGetItems](dynamodb.md#transactgetitems) is partial<br>[TransactWriteItems](dynamodb.md#transactwriteitems) is partial<br>[Design gap](design-gaps.md#dynamodb-transaction-scope-is-single-partition-single-table): Transaction scope is single-partition, single-table<br>[Design gap](design-gaps.md#dynamodb-transaction-execution-has-one-configured-cosmos-authority): Transaction execution has one configured Cosmos authority | `dynamodb_single_partition_transactions` |
...
| Multipart upload | 🟡 conditional | 6 implemented, 1 partial | 6/7 | Multipart initiation, part upload/copy/listing, in-progress upload enumeration, completion, and abort map to Azure block-blob primitives plus a bounded durable proxy-owned state index.<br>Suitable when workloads benefit from restart-stable enumeration and copy fidelity, and do not require AWS to reject stale per-part ETags with full InvalidPart fidelity.<br>[ListMultipartUploads](s3.md#listmultipartuploads) is partial<br>[Design gap](design-gaps.md#s3-multipart-upload-keeps-bounded-durable-proxy-state): Multipart upload keeps bounded durable proxy state<br>[Design gap](design-gaps.md#s3-multipart-per-part-etag-validation-cannot-be-reproduced): Multipart per-part ETag validation cannot be reproduced | `s3_multipart_upload` |
...
| FIFO topic ordering and deduplication | 🟡 conditional | 4 partial | 4/4 | A Service Bus-backed subset exists for .fifo topics, but full SNS FIFO parity is not reproduced end to end.<br>Use the Service Bus backend only, supply MessageGroupId on every publish, rely on MessageDeduplicationId or ContentBasedDeduplication within the broker's duplicate-detection window, and provision session-aware Service Bus subscriptions outside the SNS compatibility APIs when consumer-side ordering must be guaranteed.<br>[CreateTopic](sns.md#createtopic) is partial<br>[Publish](sns.md#publish) is partial<br>[PublishBatch](sns.md#publishbatch) is partial<br>[DeleteTopic](sns.md#deletetopic) is partial<br>[Design gap](design-gaps.md#sns-fifo-topics-are-deferred): FIFO topics are deferred | `sns_fifo` |
