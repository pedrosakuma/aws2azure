...
- Root `llms.txt` and `documentation-manifest.json` are deterministic discovery
  artifacts generated by `tools/Aws2Azure.Documentation`. The manifest indexes
  every stable product/developer Markdown file under its declared repository
  roots, all canonical structured sources, and stable generated
  operation/design-gap artifacts. The machine-readable coverage policy
  explicitly excludes only agent instructions and the maintainer release
  template; transient build output is never indexed. The build hook copies both
  discovery artifacts to the published site root unchanged.
...
Regenerate discovery metadata after adding, removing, or moving stable
documentation, a canonical structured source, or a generated artifact:
