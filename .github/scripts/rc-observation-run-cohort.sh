...
if [ "${1:-prepare}" = finish ]; then
  python3 .github/scripts/rc-observation-readiness.py finish
  jq -e --arg profile "$PROFILE" --arg cohort "$COHORT" '
    .schema_version == 1 and .profile.id == $profile and .profile.version == 1 and
    .cohort.role == $cohort and (.metrics | length) == 2 and
    .load_shape.candidate_concurrency > 0 and .load_shape.stable_concurrency > 0 and
    (($cohort == "candidate" and .restoration != null and .restoration.verified == true)
     or ($cohort == "stable" and .restoration == null))
  ' "$CAPTURE_ROOT/cohort-capture.json" >/dev/null
  exit 0
fi
[ "${1:-prepare}" = prepare ] || { echo "::error::Unknown cohort phase."; exit 1; }
remaining=$(( $(date -u -d "$OBSERVATION_DEADLINE_UTC" +%s) - $(date -u +%s) ))
if [ "$remaining" -le $((WINDOW_MINUTES * 60 + 1320 + 120)) ] ||
   [ "$remaining" -gt $(((WINDOW_MINUTES + 120) * 60)) ]; then
  echo "::error::Cohort end-to-end budget is expired or invalid; refusing to provision."
  exit 1
fi
...
append_env AWS2AZURE_RC_OBSERVATION_WINDOW_MINUTES "$WINDOW_MINUTES"
append_env AWS2AZURE_RC_OBSERVATION_COHORT_CAPTURE_PATH "$GITHUB_WORKSPACE/$CAPTURE_ROOT/cohort-capture.json"
append_env AWS2AZURE_RC_OBSERVATION_READINESS_DIR "$GITHUB_WORKSPACE/$CAPTURE_ROOT/readiness"
python3 .github/scripts/rc-observation-readiness.py launch
