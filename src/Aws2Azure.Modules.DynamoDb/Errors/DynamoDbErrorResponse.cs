...
public static class DynamoDbErrorResponse
{
    ...
    internal static string ComputeAwsCrc32Decimal(string payload)
    {
        uint crc = 0xFFFFFFFFu;
        ReadOnlySpan<byte> bytes = Encoding.UTF8.GetBytes(payload);
        foreach (byte b in bytes)
        {
            crc = Crc32Table[(int)((crc ^ b) & 0xFF)] ^ (crc >> 8);
        }

        return (~crc).ToString(CultureInfo.InvariantCulture);
    }
