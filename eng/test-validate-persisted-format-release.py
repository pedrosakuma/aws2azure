...
INVENTORY = REPO_ROOT / "docs/compatibility/dynamodb-persisted-formats-v3.json"
...
class PersistedFormatReleaseTests(unittest.TestCase):
    ...
    def write_notes(self, digest: str, changes: str = "None.") -> None:
        self.notes.write_text(
            ...
            f"- DynamoDB persisted-format contract: inventory `v3`, `sha256:{digest}`.\n"
    ...
    def test_exact_candidate_inventory_and_complete_declarations_pass(self) -> None:
        digest = hashlib.sha256(
            (self.root / "docs/compatibility/dynamodb-persisted-formats-v3.json")
            .read_bytes()
        ).hexdigest()
    ...
    def test_v2_baseline_inventory_accepts_additive_v3_identity_set(self) -> None:
        ...
        self.write_notes(
            digest,
            "Added atomicTransactWrite_v4 while retaining v2/v3 transaction hashes.",
        )
        self.run_tool(expect_success=True)
