...
/// proxy must reject during the SigV4 stage, paired with the AWS-contract
/// outcome (HTTP status + error <c>Code</c>) that real S3 documents for that
/// rejection. These outcomes are derived from the AWS S3 API contract, not from
    ...
    Action<HttpRequestMessage> Sign)
{
    public HttpRequestMessage BuildRequest()
    {
        var request = new HttpRequestMessage(
            HttpMethod.Get,
            new Uri("http://s3.us-east-1.amazonaws.com/conformance-bucket/key.txt"));
        Sign(request);
        return request;
    }
}
...
/// <summary>
/// The S3 proxy-side auth-error matrix. Every case rejects in the SigV4 stage
/// before any Azure call, so the whole matrix runs offline on every PR.
/// </summary>
