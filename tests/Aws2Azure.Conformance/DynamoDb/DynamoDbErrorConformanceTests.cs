...
/// <summary>
/// Tier-1 (offline, every-PR, blocking) DynamoDB error conformance. DynamoDB
/// speaks AWS JSON 1.0, so this matrix is the first to exercise the
/// JSON-envelope path of <see cref="AwsErrorCanonicalizer"/> end-to-end against
/// a real service module. For each proxy-side rejection it:
/// <list type="number">
///   <item>asserts the proxy's response matches the AWS DynamoDB
...
/// </list>
/// </summary>
public sealed class DynamoDbErrorConformanceTests : IClassFixture<DynamoDbConformanceFixture>
{
    private readonly DynamoDbConformanceFixture _fixture;

    public DynamoDbErrorConformanceTests(DynamoDbConformanceFixture fixture) => _fixture = fixture;

    public static IEnumerable<object[]> CaseNames() =>
        DynamoDbErrorMatrix.Cases.Select(c => new object[] { c.Name });
    ...
    public async Task Proxy_error_matches_aws_contract_and_golden(string caseName)
    {
        ...
        // (1) AWS-contract oracle — always enforced, offline. Pin the DynamoDB
    ...
    private static string BuildReport(
        DynamoDbErrorCase testCase, GoldenFile golden,
